...
local function PlayerOnCurrentShard(userid)
    for k, v in pairs(AllPlayers) do
        if v.userid == userid then return true end
    end
    return false
end
...
local function SendCommand(fnstr)
    ...
    local is_valid_time_to_use_remote = TheNet:GetIsClient() and
                                            TheNet:GetIsServerAdmin()
...
local TooManyItems = Class(Widget, function(self)
    ...
    local savepath = TOOMANYITEMS.TELEPORT_DATA_FILE ..
                         "toomanyitemsplus_teleport_save_" .. world_id
    ...
    elseif TOOMANYITEMS.G_TMIP_DATA_SAVE == -1 then
        _G.TheSim:GetPersistentString(savepath, function(load_success, str)
...
function TooManyItems:FlushPlayer()
    ...
    self.currentuser:SetString(string.format(prefix .. mainstr, playername,
                                             TOOMANYITEMS.CHARACTER_USERID))
end

function TooManyItems:FlushConfirmScreen(fn)
    local confirmscreen
    confirmscreen = PopupDialogScreen(fn[3], fn[4], {
        {
            ...
            cb = function() TheFrontEnd:PopScreen(confirmscreen) end
        }
    })
...
function TooManyItems:DebugMenu()
    ...
    self.currentuser:SetString(string.format(prefix .. mainstr,
                                             GetCharacterName(),
                                             TOOMANYITEMS.CHARACTER_USERID))
    self.currentusersizex, self.currentusersizey =
        self.currentuser:GetRegionSize()
    self.currentuser:SetPosition(self.left + self.currentusersizex * .5,
                                 self.shieldsize_y * .5 - self.currentusersizey *
                                     .5, 0)
    ...
    local function IsShowBeta(beta)
        if beta and BRANCH == "release" then return false end
        return true
    end
    ...
    local function IsShowButton(beta, cave)
        if IsShowBeta(beta) and IsShowPos(cave) then return true end
        return false
    end
    ...
    local function MakeDebugButtonList(buttonlist)
        for i = 1, #buttonlist do
            local tittle = self.debugshield:AddChild(
            ...
            MakeDebugButtons(buttonlist[i].list,
                             self.left + width + self.spacing)
            self.top = self.top - self.nextline
        end
    end
...
function TooManyItems:SettingMenu()
    ...
    self.screenname = self.settingshield:AddChild(
                          Text(self.font, self.fontsize * 1.5))
    self.screenname:SetColour(0, 1, 1, 1)
    ...
    self.foodfreshnessvalue = self.settingshield:AddChild(
                                  Text(self.font, self.fontsize))
    ...
    self.toolfiniteusesvalue = self.settingshield:AddChild(
                                   Text(self.font, self.fontsize))
    ...
    -- 燃料剩余度
    self.prefabfuel =
        self.settingshield:AddChild(Text(self.font, self.fontsize))
    ...
    self.prefabfuelvalue = self.settingshield:AddChild(
                               Text(self.font, self.fontsize))
    ...
    -- 温度
    self.prefabtemperature = self.settingshield:AddChild(
                                 Text(self.font, self.fontsize))
    ...
    self.prefabtemperaturevalue = self.settingshield:AddChild(
                                      Text(self.font, self.fontsize))
...
function TooManyItems:OnControl(control, down)
    if TooManyItems._base.OnControl(self, control, down) then return true end
